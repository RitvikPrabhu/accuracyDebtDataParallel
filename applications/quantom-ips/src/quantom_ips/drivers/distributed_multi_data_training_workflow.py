...
from quantom_ips.trainers.distributed_wgan_trainer import DistributedWGANTrainer
...
@dataclass
class DistributedMultiDataGANWorkflow:
...
cs.store(
    name="distributed_multi_data_gan_workflow", node=DistributedMultiDataGANWorkflow
)

@hydra.main(version_base=None, config_name="distributed_multi_data_gan_workflow")
def run(config) -> None:
