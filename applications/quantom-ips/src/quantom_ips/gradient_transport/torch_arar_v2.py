...
class TorchARARV2Defaults:
    ...
    accum_mode: str = "mean"
    gradient_trim_ratio: float = 0.25
    n_byzantine_workers: int = 1


@register_with_hydra(
    group="trainer/gradient_transport", defaults=TorchARARV2Defaults, name="torch_arar_v2"
)
class TorchARARV2:
    """
    ...
    def __init__(
        ...
        self.accum_mode = config.accum_mode
        self.gradient_trim_ratio = config.gradient_trim_ratio
        self.n_byzantine_workers = config.n_byzantine_workers
    ...
    def assign_device_to_rank(self):
        ...
        self.devices = dev
        logging.info(
            f">>> {self.name}: {self.rank} uses torch device: {self.torch_device} and is on processor: {MPI.Get_processor_name()}<<<"
        )

        if self.use_grouping == True:
            logging.info(
                f">>> {self.name}: Inner Rank: {self.inner_rank} and Outer Rank {self.outer_rank} use torch device: {self.torch_device} and is on processor: {MPI.Get_processor_name()}<<<"
            )
    ...
    def group_ranks(self, gradient_sync_mode, group_size, default_group_size):
        ...
        if (
            ...
            if group_size < 1:
                logging.warning(
                    f">>> {self.name}: The group size you provided {group_size} is < 1. Going to set it to default: {default_group_size}<<<"
                )
                self.group_size = default_group_size
            else:
                logging.info(
                    f">>> {self.name}: Running with group size: {group_size} <<< "
                )
        ...
        # Mode without grouping:
            ...
            logging.warning(
                f">>> {self.name}: The gradient synchronization mode you provided {gradient_sync_mode} is not implemented. Please check your settings <<<"
            )
    ...
    # Handle gradient accumulation: This function assumes stacked numpy arrays:
    # ****************************
    def accum_gradients(self,stacked_gradiens,num_ranks):
        if self.accum_mode.lower() == "mean":
            return np.mean(stacked_gradiens,0)
        
        if self.accum_mode.lower() == "median":
            return np.median(stacked_gradiens,0)
        
        if self.accum_mode.lower() == "trimmed_mean":
            k = int(self.gradient_trim_ratio * num_ranks)
            if k > 0:
                sorted_gradients = np.sort(stacked_gradiens,0)
                trimmed_gradients = sorted_gradients[k:-k, :]
                return np.mean(trimmed_gradients,0)
            return np.mean(stacked_gradiens,0)
        
        if self.accum_mode.lower() == "krum":
            if num_ranks > 2*self.n_byzantine_workers + 2:
                dists = np.sum((np.unsqueeze(stacked_gradiens,1) - np.unsqueeze(stacked_gradiens,2))**2,2)
                scores = []
                k = num_ranks - self.n_byzantine_workers - 2
                #+++++++++++++++
                for r in range(num_ranks):
                    sorted_dists = np.sort(dists[r])
                    scores.append(np.sum(sorted_dists[1 : k+1]))
                #+++++++++++++++
                selected = int(np.argmin(np.array(scores)))
                return stacked_gradiens[selected]
    ...
    # ****************************
    # Define ring_allreduce for a single layer:
    def ring_allreduce_single_layer(self, data, mpi_comm, num_ranks, neighbours):
        ...
        for i in range(num_ranks - 1):
            if i % 2 == 0:
                ...
                accum_data.append(recv_data[:])
            else:
                # Send recv_buff
                send_req = mpi_comm.Isend(recv_data, neighbours[1])
                mpi_comm.Recv(send_data, neighbours[0])
                accum_data.append(send_data[:])
            send_req.wait()
        # +++++++++++++++++++++++

        return np.stack(accum_data,0)
    ...
    def ring_allreduce(self, currrent_gradients, mpi_comm, num_ranks, neighbours):
        ...
        for key in currrent_gradients:
            current_gradient_stack = self.ring_allreduce_single_layer(
                currrent_gradients[key], mpi_comm, num_ranks, neighbours
            )
            new_gradients[key] = self.accum_gradients(current_gradient_stack,num_ranks)
        # +++++++++++++++++++++++
