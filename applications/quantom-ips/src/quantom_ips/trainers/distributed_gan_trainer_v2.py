...
@dataclass
class DistributedGANTrainerDefaultsV2:
    id: str = "DistributedGANTrainerV2"
...
@register_with_hydra(
    group="trainer",
    defaults=DistributedGANTrainerDefaultsV2,
    name="distributed_gan_trainer_v2",
)
class DistributedGANTrainerV2:
    ...
    def collect_predictions(self,local_prediction):
        if self.pred_rank >= 0:
           if self.config.average:
               local_prediction = local_prediction.mean(dim=0,keepdim=True)
    ...
    def discriminator_update(
        self, discriminator, discriminator_optim, real_loss, fake_loss, update_outer_group, do_update
    ):
        if do_update:
            # Compute full loss:
    ...
    def train_step(
        ...
        _, disc_opt_footprint = mon_gpu_usage(
            ...
            discriminator,discriminator_optim, real_loss, fake_loss, update_outer_group, update_disc
        )
